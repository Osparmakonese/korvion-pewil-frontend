/**
 * CounterPOS.js — Counter (Light): the Pewil "Design A" Counter layout.
 * Products grid on the left, an always-visible Current sale panel on the right
 * with big totals + tender + complete. Light, bright, friendly.
 *
...
const C = {
  ...
  line: '#e5e7eb', bg: '#f4f6f2', panel: '#ffffff', card: '#ffffff', soft: '#f0f3ee',
  amber: '#c97d1a', danger: '#c0392b',
};
      ...
      <div style={S.pos}>
        ...
        <div style={S.cart}>
          ...
          <div style={S.totals}>
            ...
            <div style={S.grand}><span style={{ fontSize: 13, fontWeight: 700 }}>Total</span><span style={{ fontSize: 26, fontWeight: 800, color: C.green }}>${fmt(grandTotal)}</span></div>
          </div>
          <div style={S.pay}>
            <div style={S.seg}>
              {tenders.map((t) => (
...
const S = {
  ...
  search: { flex: 1, border: `1px solid ${C.line}`, background: '#fff', color: C.ink, borderRadius: 11, padding: '11px 13px', fontSize: 14, outline: 'none' },
  chips: { display: 'flex', gap: 8, overflow: 'auto', paddingBottom: 12 },
  chip: { whiteSpace: 'nowrap', border: `1px solid ${C.line}`, background: '#fff', borderRadius: 999, padding: '7px 14px', fontSize: 13, fontWeight: 600, color: '#374151', cursor: 'pointer' },
  chipOn: { background: C.green, borderColor: C.green, color: '#fff' },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 12, overflow: 'auto', alignContent: 'start', flex: 1 },
  prod: { background: '#fff', border: `1px solid ${C.line}`, borderRadius: 14, padding: 12, textAlign: 'center' },
  thumb: { width: 44, height: 44, borderRadius: 11, background: C.soft, display: 'grid', placeItems: 'center', fontSize: 20, fontWeight: 800, color: C.green, margin: '2px auto 8px' },
  pname: { fontSize: 13, fontWeight: 700, minHeight: 34, color: C.ink },
  pprice: { color: C.green, fontWeight: 800, fontSize: 15 },
  pstk: { fontSize: 10.5, color: '#9ca3af', marginTop: 2 },
  add: { marginTop: 9, width: '100%', border: 0, background: C.green, color: '#fff', fontWeight: 700, fontSize: 12.5, borderRadius: 9, padding: 8, cursor: 'pointer' },
  ...
  pay: { padding: '12px 18px 16px', background: '#fff', borderTop: `1px solid ${C.line}` },
  seg: { display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6, marginBottom: 10 },
  segBtn: { border: `1px solid ${C.line}`, background: '#fff', borderRadius: 10, padding: '9px 4px', fontSize: 12, fontWeight: 700, color: '#374151', cursor: 'pointer' },
  segOn: { background: '#ecfdf5', borderColor: C.green, color: C.green },
  minor: { flex: 1, border: `1px solid ${C.line}`, background: '#fff', borderRadius: 9, padding: '9px', fontSize: 12, fontWeight: 700, color: '#374151', cursor: 'pointer' },
  complete: { width: '100%', border: 0, background: 'linear-gradient(180deg,#22844a,#1a6b3a)', color: '#fff', fontWeight: 800, fontSize: 16, borderRadius: 12, padding: 15, boxShadow: '0 8px 20px rgba(26,107,58,.25)' },
};
